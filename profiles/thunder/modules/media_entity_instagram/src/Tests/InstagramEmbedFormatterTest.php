...
class InstagramEmbedFormatterTest extends WebTestBase {
  ...
  public function testManageFieldFormatter() {
    ...
    $edit = [
      'fields[field_embed_code][settings_edit_form][settings][width]' => '400',
      'fields[field_embed_code][settings_edit_form][settings][height]' => '400',
    ];
    $this->drupalPostAjaxForm(NULL, $edit, 'field_embed_code_plugin_settings_update');
    $this->drupalPostForm(NULL, [], t('Save'));
    $this->assertText('Your settings have been saved.');
    $this->assertText('Width: 400 px');
    $this->assertText('Height: 400 px');
    ...
    // size.
    $this->assertFieldByXPath('//iframe[@width="400" and @height="400"]');
    $this->assertNoRaw('platform.instagram.com/en_US/embeds.js');

    // Now try to set formatter to embed in the responsive mode.
    $this->drupalGet('admin/structure/media/manage/' . $bundle->id() . '/display');
    $this->drupalPostAjaxForm(NULL, [], "field_embed_code_settings_edit");
    $edit = [
      'fields[field_embed_code][settings_edit_form][settings][width]' => '',
      'fields[field_embed_code][settings_edit_form][settings][height]' => '',
    ];
    $this->drupalPostAjaxForm(NULL, $edit, "field_embed_code_plugin_settings_update");
    $this->drupalPostForm(NULL, [], t('Save'));
    $this->assertText('Your settings have been saved.');
    $this->assertText('Responsive size');

    // Check if the embed appears in responsive mode.
    $this->drupalGet('media/1');
    $this->assertRaw('instagram.js');
    $this->assertRaw('jquery.responsiveinstagram.min.js');
  }
